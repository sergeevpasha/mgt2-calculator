...
- Vercel builds the Nuxt app itself from the Root Directory `src/`. The repository has no production Dockerfile, CI or `vercel.json`; do not add any unless the user asks.
- Vercel installs dependencies with Corepack, using the Yarn version and lockfile in `src/`, like the dev container. This needs the Vercel project's `ENABLE_EXPERIMENTAL_COREPACK=1` variable and the `packageManager` field of the root `package.json`: Vercel's build reads that field at the repository root, not in `src/`. Keep it equal to the one in `src/package.json`. A working build logs `Detected ENABLE_EXPERIMENTAL_COREPACK=1 and "yarn@…" in package.json`; `yarn install v1.…` means Vercel fell back to Yarn 1, which ignores `src/yarn.lock` and installs the newest versions the ranges allow. The warning about `Error: Dynamic require of "util"` comes from `"type": "module"` in `src/package.json`; builds succeed despite it.
